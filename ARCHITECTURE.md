...
| **M2.5 planning** | ✅ **PLANNING COMPLETE** (commit `3c2b49b050a2`): MILESTONE_M2.5_PLANNING.md created. Scope: 5 phases (repo setup, Gecko customization, build & test, versioning, browser-dev integration). Dependencies: code signing cert (acquire in parallel), build environment (MSVC, Rust, MozillaBuild). Estimated effort: 4–6 weeks. Blockers: none (cert deferrable to M2.5.2). Gating for M4 feature platforms. Next: assign M2.5.1 tasks, begin build environment setup. | M2.5.1 kickoff pending |
| **M3 — UI Redesign (DONE)** | Replace traditional Firefox interface: new chrome (SolidJS), tab strip, sidebar, command palette, themes engine | Visual milestone demo; build + tests green |
| **M3 final** | ✅ **COMPLETE** (commit `3aedd900d303`): Themes engine DONE (`397f3c52eb8c`): Stratus design in themes engine (skin CSS + settings option + SVGs + jar.mn + i18n + tests). Chrome redesign DONE (`ff5493e9be72`–`9666b14e90d2`): Stratus skin extended to all surfaces — command palette (container, search, categories, items, step indicator, step choices + scrollbar), panel sidebar (buttons, active pill, floating splitter hovers, header border), status bar (shared surface), find bar, tab strip (active label, close hover, pinned underline, loading, sound icon), URL bar (focus ring, icon hover), workspaces (toolbar button, selector pill, restore items), split-view (drag handles, grid handles, button). **Test coverage:** 14 assertions (`testStratusCovers*` functions) all passing. **Build verification:** 9/9 colocated designs tests, 209/209 host tests, 6/6 smoke steps, stage rebuild SUCCESS, prod bundle byte-identical. **Documentation:** MILESTONE_M3_COMPLETE.md created. Ready for M2.5 runtime rebuild. | ✅ All gates passed; M3 documentation committed |
...
| **M4 Phase 1 (vertical tabs)** | ✅ **COMPLETE** (commit `601661dd9b13`): Vertical tabs UI toggle implemented. Feature module (`vertical-tabs/index.ts`): toggle command + Ctrl+Shift+V shortcut. Config migration (`old-config-migrator.ts`): new string-based pref `floorp.tabbar.style.current` with backward compatibility to old int-based pref. **Test coverage:** 2 new tests (config persistence, backward compatibility) + 16 existing stratus tests all passing. **Build verification:** 209/209 host tests green, stage build SUCCESS. **Documentation:** M4_P1_COMPLETION.md + M4_P2_PLANNING.md created. Ready for M4 Phase 2 (workspaces/split-view hardening). | ✅ All gates passed; M4 Phase 1 complete; M4 Phase 2 planning documented |

...
| **M3 progress** | Themes engine DONE (commit `397f3c52eb8c`): Stratus design in themes engine (skin CSS + settings option + SVGs + jar.mn + i18n + tests). Chrome redesign in progress (commit `…`): Stratus skin extended to command palette, panel sidebar, status bar, find bar. Remaining: tab strip redesign, sidebar redesign, command palette redesign, visual demo | Visual milestone demo; build + tests green |

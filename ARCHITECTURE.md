...
| **M3 progress** | Themes engine DONE (commit `397f3c52eb8c`): Stratus design in themes engine (skin CSS + settings option + SVGs + jar.mn + i18n + tests). Chrome redesign in progress (commit `ff5493e9be72`): Stratus skin extended to command palette, panel sidebar, status bar, find bar. Tab strip + urlbar refinements DONE (commit `9a55fed8b2ce`): close-button surfaces, loading/sound accents, vertical-tab geometry, urlbar focus ring. Workspaces popup + split view DONE (commit `eee332931a35`): accent-selected workspace pill, restore-item surfaces, accent drag handles, split-view button radius. Palette details + sidebar splitters DONE (commit `515469ed2a66`): accent match highlight + caret + back-button hover, rounded step-error, accent splitter hovers, panel-sidebar-header accent border. Palette step choices DONE (commit `7f5d7c65bf95`): accent scrollbar for step-choices + command list, selected choice label accent, loading state styling. Remaining: visual milestone demo | Visual milestone demo; build + tests green |
